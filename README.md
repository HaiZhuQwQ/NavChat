...
NavChat 是一个用于 ChatGPT Web（网页端）的 Chrome Extension（浏览器扩展），帮助你在长对话中快速定位内容。当前版本 `v1.9`，已完成多平台架构第一阶段（core 核心层 + adapter 适配层），并优先保证 ChatGPT 现有能力不回退。

## v1.9 更新摘要

- 增强新版 ChatGPT 页面兼容性：主对话容器改为评分选择，避免命中空容器。
- 增加导航面板自愈挂载：页面重建导致面板节点丢失时会自动恢复。
- 保留收起态半隐藏图标与悬停展开效果，同时刷新后默认展开，避免误判为未加载。
- 降低调试期噪声日志级别，减少 Chrome 扩展页的非致命警告。

## v1.8 更新摘要

- 修复“从首页切换到对话后仍显示上一个对话导航”的问题（路由切换后自动重置并刷新导航数据）。
- 修复“最后一条回答需要再发一条消息才出现在导航”的问题（尾轮次补偿刷新策略增强）。
- 收起态按钮支持 PNG（位图）图标与半隐藏悬停展开效果，面板视觉更轻量。
